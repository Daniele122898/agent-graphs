...
- **Session can be rebound to a different team mid-life** via `Session.rebind()` — sets the new `team_id` + `graph`, re-seeds the agent registry (agents common to both graphs preserve history, new ones are registered as idle, removed ones are cleaned up via `AgentRegistry.unregister()`). `POST /api/sessions/{id}/rebind` (api/sessions.py) validates team existence and persists the change. This is the backend side of the team-selector "↻ Use for session" button. Reversibility: the endpoint can be removed, sessions return to pin-at-launch behavior.
