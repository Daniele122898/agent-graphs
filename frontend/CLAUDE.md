...
- **Harness toggle at launch**: Onboarding has an "Agent harness" select (native | opencode) passed to `api.launchSession`; the active session's `harness` shows as an "opencode" chip in the header when not native. The control room is harness-agnostic — both backends publish the same bus event shapes, so transcript/todos/questions/lifecycle/delegation render identically. Don't special-case the harness in render code.
