...
- **Team management lives in `ManageTeamsDialog.tsx`** (a centered modal —
  `.modal`/`.modal-overlay`, the app's first true modal; older dialogs are inline
  `.card`s). It's the ONE place to rename/describe/delete/create teams + **search**
  them, opened from the header TEAM zone's sliders `IconButton` AND from
  Onboarding (`onManage`) so teams are manageable with or without a session. It
  hosts **"+ New team"** (a blank starter team) — the only non-onboarding way to
  create a team from scratch. Rows commit name/description on blur/Enter (empty
  name reverts); delete is a two-step inline confirm and **surfaces the backend's
  block-if-bound 409 inline**. A bound team shows an "in use" chip with delete
  DISABLED (the backend is still the source of truth and 409s regardless). Teams
  carry a free-text **`description`** (`TeamRow.description`, shown as the
  `<option>` tooltip in every team selector). `api.createTeam` takes a
  `{name, description?, graph?}` body; `api.updateTeam` is the PATCH;
  `api.deleteTeam` the DELETE. App owns the `manageOpen` state + an `activeTeamId`
  reconcile so deleting the team you're *editing* falls back to the session's team
  instead of stranding the canvas on a ghost id.
- **Harness toggle at launch**: BOTH launch paths carry an "Agent harness" select (native | opencode) passed to `api.launchSession` — `Onboarding.tsx` (first session) AND `SessionSwitcher.tsx`'s "+ Session" popover (subsequent sessions). Keep them in sync: a field added to one launch form must be added to the other. **The UI default is `opencode`** (both forms' initial state + `api.launchSession`'s fallback); the hermetic verify scripts select `native` explicitly so they don't need an opencode server. The active session's `harness` shows as an "opencode" chip in the header when not native. The control room is harness-agnostic — both backends publish the same bus event shapes, so transcript/todos/questions/lifecycle/delegation render identically. Don't special-case the harness in render code.
