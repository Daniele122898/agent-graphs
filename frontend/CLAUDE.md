...
- **Edges are decorated at render time, not in the persisted graph.** `Canvas` maps every edge to the `floating` type (`FloatingEdge.tsx`: border-anchored toward the peer; reciprocal A⇄B pairs get opposite perpendicular offsets so they arc apart) and adds arrow markers — derived fresh from the full edge list each render. Keep `graphMapping`/persistence plain; node handles exist only for drag-to-connect.
